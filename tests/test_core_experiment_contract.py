...
class CoreExperimentContractTests(unittest.TestCase):
    ...
    def test_run_id_builder_and_overwrite_policy_are_wired(self):
        ...
        self.assertNotIn("SystemId", run_id_source)
        self.assertNotIn("ModelState", run_id_source)
        self.assertNotIn("config.collimator.enable", run_id_source)
        self.assertNotIn("config.vehicle.model_type", run_id_source)
