...
`MSS` 是一个 Geant4 gamma 背散射 Monte Carlo 仿真项目。项目 v2 已完成并归档，包括固定车辆 ROI + 移动成像头事件级数据生成核心，以及已经实现并冻结的 article v1 实验与后处理链路。

Geant4 核心的职责仍固定为输出事件级 `events.csv` / `events_debug.csv` 与每个 run 的 `metadata.yaml`。位姿级数据、扫描级数据、summary、统计图、差异图和指标计算不得进入 Geant4 核心；它们可以在用户明确指定的实验或后处理任务中，通过独立 Python 工具迭代。
...
```text
已归档的项目 v2 Geant4 仿真核心
+ 已实现并冻结的 article v1
+ 下一项目版本尚未开始
```

项目级分层、文档归属和脚本复用关系以 `docs/project_structure.md` 为准。
...
- Geant4 基础程序只输出事件级 CSV 与每个 run 的 `metadata.yaml`；
- 位姿级 / 扫描级数据、summary、统计图和报告由独立 Python 层产生；
- article v1 已冻结，仅用于复现、兼容性维护和迁移参考；下一项目版本必须由用户或新的正式文档明确启动。
...
2. 与任务直接对应的 v2 归档文档；未来新版本只有在正式立项文档存在时才读取其专项文档。

修改 Geant4 核心、核心配置契约或事件级输出时，还必须完整阅读：

1. `docs/archive/v2/change.md`
2. `docs/archive/v2/spec.md`
3. `docs/archive/v2/architecture.md`
4. `docs/archive/v2/decisions.md`
5. `docs/archive/v2/milestones.md`
6. `docs/archive/v2/acceptance_checklist.md`
...
`docs/archive/v2/change.md` 用于理解第二轮构建背景、第一轮与第二轮的差异、旧项目内容的参考边界。

实验层文档定位：

- article v1 冻结链路：`docs/archive/v2/article_design.md`、`docs/archive/v2/article_experiment_automation.md`。

V2 实验层文档优先级为：核心数据契约以 `docs/archive/v2/spec.md` 为准；article v1 条件设计和复现流程以 v2 归档中的 design/automation 文档为准；最后才参考现有脚本。未启动预研草案不得覆盖 v2 归档基线。

如果第一轮文档、旧代码或旧实现与第二轮文档冲突，应以第二轮文档为准，并在总结中指出冲突。

如果本文件与 `docs/archive/v2/spec.md` 或 `docs/archive/v2/decisions.md` 冲突，维护 v2 行为时应以归档规格和决策为准，并优先修订本文件。未来新版本的正式用户要求可以改变 v2 行为，但必须明确兼容性影响。
...
`docs/archive/v2/` 与 v1 archive 的性质不同：它是当前已完成代码的项目 v2 兼容基线。维护既有 Geant4 核心或复现 article v1 时必须遵守其中的稳定契约，但不得把归档文档扩写成尚未启动的新版本需求。

工作区中可能存在未启动预研文档、配置、脚本或测试。除非用户明确立项并建立新的正式规格，这些资产不得进入当前阶段描述、默认实现范围或完成性结论。
...
核心实现服务以下两条需求线。Geant4 只生成事件级数据；下列需求中的位姿级统计、扫描级统计、二维响应图、差异图和论文指标由独立后处理层实现，核心只需保留必要事件信息。
...
除非 `docs/archive/v2/spec.md` 或用户明确要求，不得主动实现：
...
以上限制约束 Geant4 核心和无明确需求的主动扩展，不禁止用户明确指定的独立实验生成器、队列、合并、分析或绘图任务。独立 Python 层不得静默改变核心 CSV schema、事件物理语义或 run/pose/seed 契约。
...
车辆 ROI 具体尺寸、材料和区域划分以 `docs/archive/v2/spec.md` 为准。
...
- 入射角 `theta` 应可配置；
- 默认参考角度以 `docs/archive/v2/spec.md` 为准。
...
具体源位置、焦点尺寸和能量模式以 `docs/archive/v2/spec.md` 与入口 YAML 为准。除入口 YAML 文件路径外，不应为这些 YAML 已覆盖的参数新增宏命令。
...
探测器 `z` 位置、接收范围和命中判定逻辑以 `docs/archive/v2/spec.md` 为准。

---

## 13. 物理过程与事件记录

必须使用 Geant4 EM physics，具体 physics list 以 `docs/archive/v2/spec.md` 为准。
...
输出事件范围以 `docs/archive/v2/spec.md` 为准。
...
当前事件追踪模型以 `docs/archive/v2/spec.md` 为准：
...
字段的最小集合、扩展集合、顺序和单位以 `docs/archive/v2/spec.md` 为准。
...
run-level 元数据写入文件名和 `metadata.yaml`。V2 不输出独立 pose-level summary 或 scan-level summary。是否写入每一行 CSV 以 `docs/archive/v2/spec.md` 为准。
...
第二轮宏命令以 `docs/archive/v2/spec.md` 为准。
...
具体命名以 `docs/archive/v2/spec.md` 为准。

---

## 17. 里程碑执行规则

Geant4 核心建设历史和 v2 维护边界以 `docs/archive/v2/milestones.md` 为基线。

若用户指定核心 milestone，Codex 每次只能实现该 milestone，不得主动实现后续 milestone。

实验生成、队列和后处理任务不强制套用已经完成的核心 M0–M16 编号；应按用户指定的实验族和范围执行，并读取对应专项文档。

如果用户要求实现某个核心 milestone 或专项实验阶段，应先确认相应输入文档是否足够明确。
...
- 不要主动实现真实探测器响应或图像重建。
- 修改实验层时不得静默改变 formal/debug CSV header、事件语义或 run/pose/seed 契约；
- article v1 已冻结，不默认扩展；未启动预研资产不得被描述为当前主线或已实现能力；
- 可以记录候选公共能力，但未经用户要求不得顺带抽取或重构全部 Python helper。

如果遇到歧义：

1. 先查 `docs/project_structure.md` 确认任务归属；
2. 核心契约问题查 `docs/archive/v2/spec.md`；
3. Article v1 复现问题查 `docs/archive/v2/article_design.md` 和 `docs/archive/v2/article_experiment_automation.md`；
4. 再查 `docs/archive/v2/decisions.md` 和 `docs/archive/v2/change.md`；
5. 若仍无法判断，暂停并列出阻塞点；
6. 不要自行替用户决定关键物理建模参数。
