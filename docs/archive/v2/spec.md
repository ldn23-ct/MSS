...
> 文档状态：项目 v2 归档的 Geant4 仿真核心最高优先级规格。
> 职责：定义配置、几何、事件追踪、CSV 和 metadata 的稳定契约。
> 上游：`change.md` 描述的第二轮重构背景。
> 下游：`decisions.md`、`architecture.md`、v2 核心实现和兼容数据消费者；项目级导航见 [`project_structure.md`](../../project_structure.md)。
...
| `pose.list.head_offset_x_mm/y_mm` | list mode 下必须长度相同；元素必须为有限数值，单位 mm，最多六位小数。 |
| `pose.grid.x_offsets_mm/y_offsets_mm` | grid mode 下元素必须为有限数值，单位 mm，最多六位小数。 |
...
offset 必须是有限数值，单位为 mm，并且最多包含六位小数。超过六位小数、NaN 或 Inf 必须 fail fast。
