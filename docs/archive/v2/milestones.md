...
> 文档状态：项目 v2 归档的 Geant4 仿真核心建设记录；已完成内容继续作为维护与回归依据。
> 职责：记录第二轮核心程序的分阶段实现顺序，不用于规划独立实验后处理。
> 上游：`spec.md`、`decisions.md` 和 `architecture.md`。
> 下游：v2 核心实现与 `acceptance_checklist.md`；项目级导航见 [`project_structure.md`](../../project_structure.md)。
...
1. 让 Codex 读取 `docs/archive/v2/spec.md`、`docs/archive/v2/decisions.md`、`docs/archive/v2/architecture.md`、`docs/archive/v2/milestones.md`。
2. 明确指定要实现的里程碑编号。
3. 明确要求不得实现后续里程碑。
4. 检查修改过的文件。
5. 运行该里程碑对应检查。
...
```text
Read docs/archive/v2/spec.md, docs/archive/v2/decisions.md, docs/archive/v2/architecture.md, and docs/archive/v2/milestones.md.
...
offset 必须是有限 mm 数值并且最多包含六位小数；不得在读取或生成 `pose_id` 时做整数截断。
...
- `README.md`
- `macros/*.mac`，如保留兼容示例
- `docs/archive/v2/acceptance_checklist.md`，如需要同步
...
```text
Review the implementation against docs/archive/v2/milestones.md and docs/archive/v2/acceptance_checklist.md.
