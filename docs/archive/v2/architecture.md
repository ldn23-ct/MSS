...
> 文档状态：项目 v2 归档的 Geant4 仿真核心架构基线。
> 职责：描述 C++ 模块边界、数据流和 run/pose 生命周期，不覆盖独立 Python 后处理架构。
> 上游：`spec.md` 和 `decisions.md`。
> 下游：v2 核心 C++ 实现、`milestones.md` 和 `acceptance_checklist.md`；项目级导航见 [`project_structure.md`](../../project_structure.md)。
...
offset 使用有限 mm 数值并支持最多六位小数。每个 pose run 使用一个实际 `random_seed`，默认可按 `base_random_seed + pose_index` 生成。
