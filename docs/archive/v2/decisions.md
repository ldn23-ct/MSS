...
> 文档状态：项目 v2 归档的 Geant4 仿真核心决策记录。
> 职责：固化已接受、取代或推迟的核心设计选择。
> 上游：`spec.md` 的正式需求和 `change.md` 的重构背景。
> 下游：`architecture.md`、`milestones.md` 和 v2 核心实现；项目级导航见 [`project_structure.md`](../../project_structure.md)。
...
| D005 | pose 支持 list / grid 两种生成模式，`pose_id` 自动生成 | Superseded |
...
| D029 | pose offset 支持最多六位小数并使用规范化 `pose_id` 编码 | Accepted |
...
`pose_id` 不由用户直接设置。本决策中的整数限制已由 D029 取代。
...
- list / grid 语义和 `pose_id` 自动生成原则保持有效；offset 数值与编码限制以 D029 为准。
...
## D029：pose offset 支持最多六位小数并使用规范化 `pose_id` 编码

**状态：** Accepted
**日期：** 2026-07-15
**取代：** D005 中的整数 offset 限制

### 背景

源项响应实验需要使用 `2.5 mm` 间隔的局部网格，其中包含 `±7.5 mm` 和 `±2.5 mm`。原有整数限制会使配置读取失败，实验队列中的整数转换还会造成静默截断。

### 决策

list / grid pose offset 接受有限数值，单位仍为 mm，并限制为最多六位小数。超过六位小数、NaN 或 Inf 必须 fail fast。

`pose_id` 对每个 offset 使用规范化十进制编码：
...
因此 `(-7.5, 2.5)` 的 `pose_id` 为 `pose_xm7p5_y2p5`。C++ 运行端与 Python 队列必须使用同一精度验证和编码规则。

### 影响

- `PoseRawConfig` 与 `ScanPose` 的 offset 类型改为 `double`。
- source、collimator 和 detector 使用未经整数截断的实际 offset。
- metadata 中的 `head_offset_x_mm` / `head_offset_y_mm` 为数值，不再限定为整数。
- 整数 pose 的现有 ID 与输出目录保持完全兼容。
