...
> 文档状态：项目 v2 归档的 Geant4 仿真核心验收基线。
> 职责：验证核心程序的构建、几何、事件级输出、多线程和错误处理。
> 上游：`spec.md`、`decisions.md` 和 `architecture.md`。
> 下游：v2 核心回归检查；实验脚本使用各自测试和专项文档验收，项目级导航见 [`project_structure.md`](../../project_structure.md)。
...
pose:
  mode: list
  list:
    head_offset_x_mm: [-7.5, -2.5, 2.5, 7.5]
    head_offset_y_mm: [2.5, 0, -2.5, 7.5]
```
...
- metadata 保留实际小数 offset。
- source、collimator 和 detector 使用相同的小数 offset。
- C++ 与 Python 队列生成完全相同的 `pose_id` 和 run directory。
