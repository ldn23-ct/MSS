...
| D003 | 探测器建模为理想计数平面 | Superseded |
| D004 | 准直器几何由外部 CSV profile 定义 | Superseded |
| D005 | 使用 `G4ExtrudedSolid` 构建两块钨准直器 jaw | Superseded |
...
| D014 | 非法准直器 profile 必须 fail fast | Superseded |
...
| D020 | 三块凸多边形 jaw、镜像准直器和镜像探测器 | Accepted |
| D021 | 验证导向的准直器开关和 debug 初始方向字段 | Accepted |
...
本决策关于“单个探测区域”的边界定义已被 D020 取代；“理想计数平面、不建模真实探测器响应”的约束仍保留。
...
本决策关于“两块 jaw、每块五个顶点”的内容已被 D020 取代；“外部 CSV profile 定义准直器、单次选择一个 profile”的约束仍保留。
...
本决策关于“两块凸五边形 jaw”的内容已被 D020 取代；`G4ExtrudedSolid` 和坐标映射约束仍保留。
...
- Codex 不得新增 compact CSV 列；debug CSV 列只能随 `spec.md` 的明确修订更新。
- 输出文件名是数据契约的一部分。
...
- 固定几何常量应集中定义并易于检查。
- Codex 不得新增额外宏命令，除非 `spec.md` 有明确修订。
...
本决策关于“两块 jaw、五个顶点、五边形非凸”的具体规则已被 D020 取代；fail fast 原则仍保留。
...
准直器 profile 需要支持三块钨板，并且钨板截面不再限制为五边形。同时仿真几何需要包含关于 `x = 0` 的镜像准直器和镜像探测区域，但不能破坏既有 CSV schema。

### 决策

每个 collimator profile 必须包含 `jaw_0`、`jaw_1`、`jaw_2` 三块 jaw。每块 jaw 是全局 x-z 平面内的凸多边形，顶点数不固定但必须 `N >= 3`，`vertex_id` 必须是 `0..N-1` 连续整数。

程序同时构建原始准直器和镜像准直器。镜像平面为 `x = 0`，即 `x' = -x, y' = y, z' = z`。每块 jaw 仍使用 `G4ExtrudedSolid`，不额外叠加 z 偏移。

程序同时使用原始探测器和镜像探测器：

```text
original: z = -73 mm, x = [53, 161] mm, y = [-50, 50] mm
mirror:   z = -73 mm, x = [-161, -53] mm, y = [-50, 50] mm
```

命中任一探测区域都按现有 detector hit 逻辑记录。CSV schema 保持不变，不新增 detector ID 或 mirror flag；镜像探测器命中时 `det_x` 保留负的全局 x 坐标。

### 理由

三块凸多边形 jaw 能表达新的准直器截面需求；镜像几何与镜像探测区域允许在同一 run 中统计两侧返回信号，同时保持 CSV 契约稳定。

### 影响

- `CollimatorProfileReader` 需要验证三块凸多边形 jaw。
- `CollimatorBuilder` 需要构建 3 块原始 jaw 和 3 块镜像 jaw。
- `DetectorConstruction` 需要显示两个探测面辅助体。
- `SteppingAction` 需要接受两个探测区域。
- 真实探测器响应、sensitive detector、能量沉积 scoring 仍不属于第一版范围；debug CSV 字段扩展见 D021。

---

## D021：验证导向的准直器开关和 debug 初始方向字段
...
需要验证无准直器时的几何与输出链路，并需要在 debug CSV 中检查 primary gamma 的初始发射方向。同时发现 `initial_energy` 若由 `PrimaryGeneratorAction` 直接写入 `EventAction`，会被 event begin 阶段 reset 覆盖。
...
默认值为 `true`。为 `false` 时，`DetectorConstruction` 不读取 collimator profile、不调用 `CollimatorProfileReader`、不调用 `CollimatorBuilder`，也不构建 tungsten jaw；World、PMMA、空气缺陷和探测面辅助体保持不变。

debug CSV 在 `initial_energy` 后增加：

```csv
initial_dir_x,initial_dir_y,initial_dir_z
```

compact CSV schema 保持不变。`initial_energy` 与初始方向由 `EventAction::BeginOfEventAction` 从 Geant4 event 的 primary vertex 读取，避免依赖 `PrimaryGeneratorAction` 到 `EventAction` 的调用时序。

### 影响

- `SimulationConfig` 保存 `enableCollimator`。
- `SimulationMessenger` 提供 `/geometry/enableCollimator`。
- `CsvWriter` 只扩展 debug header 与 debug row。
- output file naming、多线程输出策略、探测器穿越逻辑和散射计数逻辑不变。
